...
/// <summary>
/// Gerencia as entradas de navega��o de UI (esquerda/direita/cima/baixo,
/// confirmar e cancelar), com tudo configur�vel via Inspector.
/// 
/// Pain�is como InventoryUI, PauseMenuController, MainMenuController
/// chamam os m�todos deste script em vez de ler Input direto.
/// </summary>
public class UINavigationInput : MonoBehaviour
{
    public static UINavigationInput Instance { get; private set; }

    [Header("Teclas de navega��o (teclado)")]
    ...
    [Tooltip("Valor m�nimo do eixo para considerar como navega��o (0.5 recomendado).")]
    [SerializeField] private float axisThreshold = 0.5f;

    [Header("Bot�es de a��o")]
    [Tooltip("Tecla de confirmar no teclado (ex.: Q, Enter, Space).")]
    [SerializeField] private KeyCode confirmKeyKeyboard = KeyCode.Q;

    [Tooltip("Bot�o de confirmar no controle (mapeado no Input Manager, ex.: Submit).")]
    ...
    [Tooltip("Bot�o de cancelar/voltar no controle (ex.: Cancel, B).")]
    [SerializeField] private string cancelButtonGamepad = "Cancel";

    [Header("Intera��o de gameplay")]
    [Tooltip("Tecla de intera��o no teclado (ex.: E).")]
    [SerializeField] private KeyCode interactKeyKeyboard = KeyCode.E;

    [Tooltip("Bot�o de intera��o no controle (ex.: Submit).")]
    [SerializeField] private string interactButtonGamepad = "Submit";

    // Controle interno para evitar repeti��o quando o eixo fica segurado
    private bool horizontalAxisInUse = false;
    private bool verticalAxisInUse = false;

    // Flags para desativar uso de eixo caso o Input Manager n�o tenha o axis
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogWarning("[UINavigationInput] J� existe uma inst�ncia, destruindo duplicata.");
            Destroy(gameObject);
            return;
        }
    ...
    // ------------------ M�TODOS P�BLICOS PARA OS PAIN�IS ------------------
    ...
    private float GetHorizontalAxisSafe()
    {
        ...
        {
            Debug.LogWarning($"[UINavigationInput] Axis '{horizontalUIAxis}' n�o est� configurado no Input Manager. Desativando uso de eixo horizontal para UI.\n{e.Message}");
    ...
    private float GetVerticalAxisSafe()
    {
        ...
        {
            Debug.LogWarning($"[UINavigationInput] Axis '{verticalUIAxis}' n�o est� configurado no Input Manager. Desativando uso de eixo vertical para UI.\n{e.Message}");
